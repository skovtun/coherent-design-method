...
const SYSTEM_PROMPT = [
  ...
  '   BRAND is the single signature color a person would name as "this site\'s color" (e.g. Stripe indigo, Spotify green).',
  '   Choose it by SALIENCE and saturation, NOT by raw frequency or CTA usage: a vivid, distinctive hue that appears only',
  '   in the logo, icons, illustrations, or link/hover accents is still the brand color, and outranks a high-frequency',
  '   neutral like black or white. Label AT MOST ONE hex brand, and it should be chromatic — pick a near-black/near-white',
  '   neutral as brand only when the palette is genuinely monochrome with no saturated color at all.',
  '   ACCENT = other saturated highlights beyond the brand color. NEUTRAL = grays and off-whites. SEMANTIC = success/',
  '   warning/error/info. TEXT = colors used on copy. BORDER = thin outlines.',
  '   BACKGROUND = page/section/card FILLS, which are near-neutral (near-white, near-black, or a soft desaturated tint).',
  '   A highly saturated chromatic color is NEVER a background even if the CSS shows it used as one (e.g. a colored',
  '   link-hover fill) — such a color is brand or accent, not background.',
