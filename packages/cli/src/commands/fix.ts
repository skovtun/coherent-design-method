 ...
 *   --journal     Write session summary to .coherent/fix-sessions/ for later review
 */

import chalk from 'chalk'
import { readdirSync, readFileSync, existsSync, rmSync, mkdirSync, writeFileSync } from 'fs'
...
export async function fixCommand(opts: FixOptions = {}) {
  ...
  // ─── Journal: capture this session for later wiki reflection ───────
  // Writes a small YAML file under .coherent/fix-sessions/ with the
  // validator types that fired + counts + sample files. Goal: let
  // future wiki curation aggregate across many sessions and surface
  // recurring patterns worth a PATTERNS_JOURNAL entry. Opt-in via
  // --journal so default runs don't litter the project.
  if (opts.journal && !dryRun) {
    try {
      const sessionsDir = resolve(projectRoot, '.coherent', 'fix-sessions')
      mkdirSync(sessionsDir, { recursive: true })
      const timestamp = new Date()
        .toISOString()
        .replace(/[:.]/g, '-')
        .replace(/-\d{3}Z$/, 'Z')
      const sessionPath = resolve(sessionsDir, `${timestamp}.yaml`)

      const groupByType = (severity: 'error' | 'warning' | 'info') => {
        const byType: Record<string, { count: number; samples: Array<{ path: string; line: number }> }> = {}
        for (const { path, issues } of fileIssues) {
          for (const issue of issues) {
            if (issue.severity !== severity) continue
            if (!byType[issue.type]) byType[issue.type] = { count: 0, samples: [] }
            byType[issue.type].count++
            if (byType[issue.type].samples.length < 5) {
              byType[issue.type].samples.push({ path, line: issue.line })
            }
          }
        }
      ...
      const renderTypes = (byType: ReturnType<typeof groupByType>, indent: string) => {
        const rows = Object.entries(byType).sort((a, b) => b[1].count - a[1].count)
        if (rows.length === 0) return `${indent}[]`
        return rows
          .map(([type, info]) => {
            const samples = info.samples
              .map(s => `${indent}    - { path: ${yamlEscape(s.path)}, line: ${s.line} }`)
              .join('\n')
            return `${indent}- type: ${type}\n${indent}  count: ${info.count}\n${indent}  samples:\n${samples}`
          })
          .join('\n')
      }

      const coherentVersion = (() => {
        try {
          const pkg = JSON.parse(readFileSync(resolve(projectRoot, 'package.json'), 'utf-8'))
          return pkg.dependencies?.['@getcoherent/cli'] || pkg.devDependencies?.['@getcoherent/cli'] || 'unknown'
        } catch {
          return 'unknown'
        }
      })()
      ...
      const lines = [
        '# coherent fix session — hypothesis data for wiki curation',
        '# Generated by `coherent fix --journal`. Do not edit by hand; aggregate via wiki tooling.',
        `timestamp: ${new Date().toISOString()}`,
        `coherent_version_in_project: ${yamlEscape(String(coherentVersion))}`,
        'auto_fixed:',
        ...fixes.map(f => `  - ${yamlEscape(f)}`),
        `totals:`,
        `  errors: ${totalErrors}`,
        `  warnings: ${totalWarnings}`,
        `  info: ${totalInfo}`,
        'remaining_errors:',
        renderTypes(groupByType('error'), '  '),
        'remaining_warnings:',
        renderTypes(groupByType('warning'), '  '),
        'remaining_info:',
        renderTypes(groupByType('info'), '  '),
        '',
      ]
      writeFileSync(sessionPath, lines.join('\n'), 'utf-8')
      console.log(chalk.dim(`\n  📝 Session journaled → ${relative(projectRoot, sessionPath)}`))
    } catch (err) {
      console.log(
        chalk.yellow(
          `  ${'\u26A0'} Could not write journal entry: ${err instanceof Error ? err.message : String(err)}`,
        ),
      )
    }
  } else if (!opts.journal && !dryRun && (totalErrors > 0 || totalWarnings > 0)) {
    // One-line hint so users discover the feature.
    console.log(chalk.dim('\n  ℹ Run with --journal to capture this session for later review.'))
  }
