 ...
 */
export async function chatCommand(message: string | undefined, options: { provider?: string; component?: string; page?: string; token?: string; interactive?: boolean; _throwOnError?: boolean }) {
  // Interactive mode: delegate to REPL
  if (options.interactive) {
    return interactiveChat(options)
  }
  ...
  try {
    ...
    // Resolve target flags (--component, --page, --token) into enhanced message
...
/**
 * Resolve --component, --page, --token flags into an enhanced message
 * that gives the AI precise context about what to modify.
 */
async function resolveTargetFlags(
  message: string,
  ...
  if (options.page) {
    ...
    const page = config.pages.find(
      p => p.name.toLowerCase() === target.toLowerCase() ||
           ...
           p.route === target || p.route === '/' + target
    )
    if (page) {
      const relPath = page.route === '/'
        ? 'app/page.tsx'
 ...
 * Keeps project context loaded across multiple commands.
 */
async function interactiveChat(options: { provider?: string }) {
  ...
  rl.on('line', async (line: string) => {
      ...
      if (manifest.shared.length === 0) {
        ...
        sorted.forEach(entry => {
          const usage = entry.usedIn.includes('app/layout.tsx')
            ...
            : entry.usedIn.length > 0 ? chalk.gray(entry.usedIn.join(', ')) : chalk.gray('unused')
          console.log(
    ...
    if (lower === 'status') {
      ...
      console.log(chalk.dim(`  Pages: ${currentConfig.pages.length}  |  Shared components: ${manifest.shared.length}  |  UI components: ${cm.getAllComponents().length}\n`))
      rl.prompt()
      return
    }
    ...
    // @Component or @/route target shortcuts
    let resolvedInput = input
    let extraOpts: { component?: string; page?: string } = {}
    ...
    try {
      await chatCommand(resolvedInput, { provider, interactive: false, _throwOnError: true, ...extraOpts })
      await dsm.load()
    } catch (err: any) {
      if (!err._printed) {
        console.error(chalk.red(`\n  Error: ${err.message}\n`))
