...
- **\`<Button>\` inside \`.map()\` MUST have explicit \`variant\`** (v0.14.1 critical). Default variant = \`bg-primary text-primary-foreground\` — every mapped item renders solid brand color regardless of what className adds (className APPENDS, doesn't override variant). For list rows, calendar cells, notification items, sidebar links: \`variant="ghost"\`. For action toggles: \`variant={isActive ? 'default' : 'outline'}\`. BAD: \`{items.map(i => <Button onClick={...} className="hover:bg-muted">{i.text}</Button>)}\` — every Button is bg-primary, hover:bg-muted never triggers. GOOD: \`{items.map(i => <Button variant="ghost" onClick={...} className="w-full justify-start">{i.text}</Button>)}\`. This is the #1 generated-app visual bug class (Calendar with all-days-blue, Notifications with stuck-on selection both reproduced 2026-04-28).
