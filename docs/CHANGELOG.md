...
- **Skill orchestrator namespaces its tmp files by session UUID** (`packages/cli/src/utils/claude-code.ts`). The `/coherent-generate` skill markdown previously piped prompts/responses through fixed paths like `/tmp/plan-prompt.md`, `/tmp/plan-response.md`, etc. Two concurrent `/coherent-generate` runs in different Coherent projects (two Claude Code tabs) would cross-contaminate: project A's response could land in project B's `_phase ingest`. The project-scoped lock does not help — it's scoped per-project, not per-tmp-file. Fix: `/tmp/coherent-$UUID-<phase>-<direction>.md`.
